...
class MLP:
    def __init__(self, res_checkpoint_dir):
        self.imgs = tf.placeholder(tf.float32, [None, None, None, None])
        ...
        self.grads = [(g, v) for (g, v) in self.grads if self.grad_filter(block_k=4, v=v)]
    ...
    def grad_filter(self, block_k, v):
        return True if 'block{}'.format(block_k) in v.name or 'classifier' in v.name else False
    ...
    def feature_extractor(self):
        ...
        _, endpoints = network_fn(preprocessed_input)
